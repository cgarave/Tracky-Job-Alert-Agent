...
export function DeleteConfirmModal({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && !isDeleting && onClose()}>
      <DialogContent className="max-w-md bg-white border-slate-200 text-slate-900 p-6 shadow-2xl rounded-2xl">
        <DialogHeader>
        ...
        <div className="flex flex-col gap-4 py-2">
          {singleJobTitle && (
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-800 font-medium truncate">
              {singleJobTitle}
            </div>
          )}
        ...
        <DialogFooter className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
          ...
          <Button
            ...
            disabled={isDeleting}
            className="text-xs gap-2 bg-rose-600 hover:bg-rose-700 text-white font-semibold shadow-sm"
          >
            {isDeleting ? (
